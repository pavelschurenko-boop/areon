...
- The worker validates the contractor code is numeric before forwarding it,
  ...
  (VAT payer status), `GET /history/{code}` (change history for name,
  contacts, managers, main economic activity, founders, and charter
  capital), and `GET /licenses?contractorCode={code}` (licenses, with
  optional `top`/`skip`/`registers`/`onlyActive` passed through after
